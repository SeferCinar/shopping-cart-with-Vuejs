...
<div>
    ...
    <div class="products">
        <div v-for="(product,index) in products" :key="index">
         ...
         <input type="number" :id="'quantity_' + index" v-model="product.quantity" min="1" /> 
         <button v-on:click="addItemToCart(product)">Sepete Ekle</button>
        </div>
    </div>
</div>
</template>
...
input[type="number"] {
  width: 80px; /* Input alanının genişliği */
